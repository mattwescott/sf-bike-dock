...
/**
 * @ngdoc service
 * @name sfBikeDock.geocode
 * @description
 * # geocode
 * Service in the sfBikeDock.
 */
angular.module('sfBikeDock')
  .service('geocode', function ($localStorage, $q, $timeout) {
    ...
    var executeNext = function () {
      ...
      geocoder.geocode({ address : task.address }, function (result, status) {
        if (status === google.maps.GeocoderStatus.OK) {
          ...
          console.log('Geocode: Over Query Limit.');
          queryPause += 250;
          $timeout(executeNext, queryPause);
        }
        else if (status === google.maps.GeocoderStatus.REQUEST_DENIED) {
          console.log('Geocode: Request Denied.');
          queue.shift();
          task.d.reject({
            type: 'denied',
    ...
    return {
      latLngForAddress : function (address) {
        ...
        if (_.has(locations, address)) {
          //console.log('GeoCode: Found in Local Storage.');
          $timeout(function () {
            d.resolve(locations[address]);
          });
        }
        else {
          //console.log('GeoCode: Querying Google.');
          queue.push({
            address: address,
            d: d
          });
